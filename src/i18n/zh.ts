...
export const messages: Record<string, string> = {
  ...
  'trigger_user_auth.denied_you_link': 'botmux: 这一步要用「发消息的你」本人的 {tool}（{provider}）授权，而你还没授权，所以命令没有执行（不会用别人或机器人的身份代跑）。请点开下面的链接、用你自己的账号确认授权：',
  'trigger_user_auth.denied_named_link': 'botmux: 这一步需要「{name}」本人的 {tool}（{provider}）授权，但还没授权，命令没有执行。请点开下面的链接、用本人账号确认授权：',
  'trigger_user_auth.denied_link_footer': '✅ 只需授权这一次：授权后这类操作以后都用你的身份自动执行，不用每次点。{persistence}完成后直接让我重试刚才的操作即可（不用再发命令）。不点链接 = 不授权 = 这类需要你本人权限的操作不会执行。',
  'trigger_user_auth.auth_persistence_lark': '授权一次后约 7 天内有效、期间自动续期，期间不用再扫码；过期后才需要再点一次。',
  'trigger_user_auth.auth_persistence_bytedcli': '授权一次后登录态约 3 周内有效、期间自动续期，期间不用再扫码；过期后才需要再扫一次。',
  'cmd.login.title': '🔐 飞书用户授权',
  'cmd.login.lark_title': '🔐 lark-cli 授权（扫码/点链接）',
  'cmd.login.lark_step1': '1. 打开下面的链接，用你自己的飞书账号确认授权：',
  'cmd.login.lark_step2': '2. 授权完成后，回到本话题发一条 /login done（或 /login 完成）',
  'cmd.login.lark_note': '说明：这是 lark-cli 通用应用的授权，不依赖本 bot 应用的可用范围——群里任何人都能自己授权。只需授权一次：之后你在本话题触发的 lark-cli 操作（读文档/云空间/Wiki/表格等）都自动用你自己的权限，约 7 天内自动续期、不用反复扫码（过期后再点一次即可）。',
  'cmd.login.lark_pending': '⏳ 还没检测到授权完成。请先点上面那条链接确认授权，再发 /login done。',
  'cmd.login.lark_ok': '✅ lark-cli 授权成功，之后本话题里你触发的 lark-cli 操作都会用你自己的权限。',
  'cmd.login.lark_failed': '❌ lark-cli 授权失败：{detail}。可以重新发 /login 再试一次。',
  'cmd.login.lark_no_challenge': '❌ 没有正在进行的 lark-cli 授权。请先发 /login 拿授权链接。',
  'cmd.login.lark_begin_failed': '❌ 无法发起 lark-cli 授权：{detail}',
  'cmd.login.lark_status_yes': 'lark-cli（你的飞书身份）：已授权',
  'cmd.login.lark_status_no': 'lark-cli（你的飞书身份）：未授权 —— 发 /login 点链接完成授权',
  ...
  'ai.credentials.on_auth_link': '如果命令被拒、stderr 里直接给了一个授权链接（accounts.feishu.cn/device/verify 或 ByteCloud 链接）和「点开授权后让我重试」的说明：立刻用 botmux send 把那段说明和链接原样转发给用户（链接逐字转发、不要改写/编码/加标点，说明里通常含「只需授权一次、约 N 天自动续期」的提示，一并保留以安抚用户只需点这一次），然后停下等用户授权，不要用任何其它身份重试、也不要假装操作成功。用户授权后会再叫你，到时重试即可。',

...
export const messages: Record<string, string> = {
  ...
  'card.traex_init.mode_title': '🚀 选择 TraeX 启动方式',
  'card.traex_init.mode_intro': '工作目录已确认，请选择启动方式。',
  ...
  'card.traex_init.mode_placeholder': '选择启动方式',
  ...
  'card.traex_init.mode_selected_with_mode': '✅ 已选择启动方式：{mode}',
  ...
  'cmd.reply_mode.status': '当前群回复模式：{mode}\n可用命令：/reply-mode chat｜chat-topic｜topic｜shared｜new-topic（new-topic 仍作为 topic 的兼容别名）',
  ...
  'cmd.reply_mode.usage': '用法：/reply-mode chat｜chat-topic｜topic｜shared｜new-topic\nchat=群内平铺回复且复用群会话（连原生话题也并进这个会话）；chat-topic=顶层平铺复用群会话，但群里原生话题各自独立会话（默认）；topic=每次 @ 都开一个独立话题+独立会话（new-topic 是兼容别名）；shared=回复落在话题里但复用同一个群 session。原生话题在 chat-topic 与 topic 下各自独立，在 chat / shared 下并进群会话。',
  ...
  'daemon.topic_header_usage': '用法：`[标题] /t [/repo 仓库] [/model 模型] [/effort 档位] 首轮任务`（换行等价于空格；带空格的路径用双引号）。',
  ...
  'daemon.complete_traex_init_first': '请先完成上方 TraeX 初始化流程。新消息已暂存，启动时会一并发送。',

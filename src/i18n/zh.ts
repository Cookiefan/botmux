...
export const messages: Record<string, string> = {
  ...
  'cmd.login.lark_step2': '2. 授权完成后，回来发一条 /login done',
  ...
  'cmd.login.lark_no_challenge': '❌ 没有正在进行的 lark-cli 授权。请先发 /login lark 拿授权链接。',
  ...
  'ai.credentials.on_auth_failure': '遇到鉴权失败：直接把失败原样告诉用户。ByteCloud 缺授权时 stderr 会附自动生成的登录链接，直接把链接给用户，授权后重试原操作；只有未返回链接时才提示 /login bytedcli。飞书授权仍使用 /login。不要试图翻找、拼凑或复用其他凭证来绕过。',
  // missing_scope 是「授权了但这一项没批」，跟「没授权」是两回事：不要让用户重跑 /login，
  // 那只会拿到同样的权限再失败一次。飞书已经把缺的 scope 名字列出来了，照抄即可。
  'ai.credentials.on_auth_link': '如果命令被拒、stderr 里直接给了授权链接（accounts.feishu.cn/device/verify 或 ByteCloud/cloud.bytedance 链接）和「点开授权后重试」说明：立刻用 botmux send 把那段说明和链接原样转发给用户（链接逐字转发、不改写/编码/加标点；保留「只需授权一次、自动续期」的提示让用户知道只点这一次），然后停下等授权，不要换其它身份重试、不要假装成功。用户授权后会再叫你，到时重试即可。',

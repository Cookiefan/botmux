...
export const messages: Record<string, string> = {
  ...
  'trigger_user_auth.denied_link_footer': '✅ You only authorize once: afterwards these actions run automatically as you — no prompt each time. {persistence}When done, just ask me to retry (no command to type). Not opening the link means not authorizing, and actions needing your own permissions will not run.',
  'trigger_user_auth.auth_persistence_lark': 'One authorization stays valid for about 7 days and renews automatically during that time — no re-scan; you only tap again after it expires. ',
  'trigger_user_auth.auth_persistence_bytedcli': 'One authorization keeps the login for about 3 weeks and renews automatically during that time — no re-scan; you only scan again after it expires. ',
  ...
  'cmd.login.lark_step2': '2. After authorizing, send /login done back in this topic',
  'cmd.login.lark_note': 'This authorizes lark-cli\'s general-purpose app and does not depend on this bot app\'s availability scope — anyone in the group can self-authorize. You authorize only once: afterwards lark-cli actions you trigger (docs / drive / wiki / sheets) run automatically as you, auto-renewing for about 7 days with no re-scan (tap again only after it expires).',
  'cmd.login.lark_pending': '⏳ Authorization not detected yet. Open the link above to confirm, then send /login done.',
  'cmd.login.lark_ok': '✅ lark-cli authorized. lark-cli actions you trigger in this topic now run with your own permissions.',
  'cmd.login.lark_failed': '❌ lark-cli authorization failed: {detail}. Send /login again to retry.',
  'cmd.login.lark_no_challenge': '❌ No lark-cli authorization in progress. Send /login first to get a link.',
  'cmd.login.lark_begin_failed': '❌ Could not start lark-cli authorization: {detail}',
  'cmd.login.lark_status_yes': 'lark-cli (your Lark identity): authorized',
  'cmd.login.lark_status_no': 'lark-cli (your Lark identity): not authorized — send /login and open the link',
  ...
  'ai.credentials.on_auth_link': 'If a command is refused and stderr contains an authorization link (accounts.feishu.cn/device/verify or a ByteCloud link) plus instructions to authorize and retry: immediately relay that text and the link verbatim to the user with botmux send (forward the link byte-for-byte; do not rewrite, encode, or add punctuation; keep the accompanying "authorize once, auto-renews for ~N days" note so they know it is a one-time tap), then stop and wait. Do not retry under any other identity and do not pretend the operation succeeded. The user will call you again after authorizing; retry then.',

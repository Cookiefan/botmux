...
export const messages: Record<string, string> = {
  ...
  'card.traex_init.mode_placeholder': 'Select a startup mode',
  ...
  'cmd.reply_mode.status': 'Current group reply mode: {mode}\nCommands: /reply-mode chat | chat-topic | topic | shared | new-topic (new-topic remains a compatibility alias for topic)',
  ...
  'cmd.reply_mode.usage': 'Usage: /reply-mode chat | chat-topic | topic | shared | new-topic\nchat = flat replies reusing the group session (native topics fold in too); chat-topic = flat at top level, but each native topic runs its own independent session (default); topic = each @ opens a fresh topic + its own session (new-topic is a compatibility alias); shared = topic display while reusing the same group session. Native topics run their own session under chat-topic and topic; they fold into the group session under chat / shared.',

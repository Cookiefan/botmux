...
export const messages: Record<string, string> = {
  ...
  'help.topic': '[title] /t [/repo <repo> | /repo wt <repo> [branch]] [/model <model>] [/effort <level>] [<first task>]  (alias /topic) - Start a topic in a regular group, declaring title, repo, model, reasoning effort and the first task in one message. Newlines are the same as spaces; the title goes BEFORE /t (Lark shows the raw message in its topic list and the bot cannot rewrite it); quote paths containing spaces; one bad field voids the whole header and replies with a usage error. Bare /t opens setup (repo picker when needed, otherwise waits for the next task or /repo); a bare /repo inside the header (no argument) starts right away in the default working dir, same as the picker card start-directly button. /repo wt <repo> [branch] creates a fresh worktree on that repo before starting: the branch may be omitted (auto-named from the title/task); when given it is only taken from the next word on the same line as the repo that looks like a branch name, so start a latin first task on a new line. Note that /repo inside the header takes a single token, while a standalone mid-session /repo still takes the rest of the line',
  ...
  'xpi.choice.suggestion': 'Leave it for the current task',
  'xpi.card.classify.prompt': '{at} Another member\'s task is still running. Choose how to handle this message:',
  'xpi.card.classify.independent': 'Start a new task',
  'xpi.card.classify.suggestion': 'Leave it for the current task',
  'xpi.card.wait.prompt': '{at} The original task is still running. Choose:',
  'xpi.card.wait.continue': 'Keep waiting',
  'xpi.card.wait.independent': 'Start a new task',
  'xpi.notice.staged': 'Message saved; it will not interrupt the current task.',
  ...
  'daemon.cascade_busy': '⚠️ The previous cascade is still running; send this once it finishes.',
  'daemon.cascade_failed': '⚠️ The cascade was interrupted: {done} item(s) were sent, {remaining} were not. Resend only the undelivered part, not the whole message.',
  'daemon.choose_repo_no_card': 'Pick a repository first with `/repo <path|project name>` or `/repo wt <repo> [branch]` in this topic; your message is queued and will be sent once a repository is chosen.',
  ...
  'daemon.topic_header_usage': 'Usage: `[title] /t [/repo <repo> | /repo wt <repo> [branch]] [/model <model>] [/effort <level>] <first task>` (newlines are the same as spaces; quote paths containing spaces; the optional `wt` branch is only taken from the next word on the same line as the repo when it looks like a branch name — start a latin first task on a new line).',
  ...
  'daemon.topic_header_worktree_missing_target': '`/repo wt` is missing its target repo: `/repo wt <path|project name> [branch]`',
  'daemon.topic_header_repo_not_git': '`{arg}` is not a git repository, so no worktree can be created there',
  'daemon.topic_header_branch_invalid': '`{arg}` is not a valid branch name (`git check-ref-format` rejects it)',
  'daemon.topic_header_worktree_exists': 'The worktree target directory already exists: `{path}` (pick another branch name, or open it directly with `/repo {path}` inside the topic)',

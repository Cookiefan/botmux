...
export const messages: Record<string, string> = {
  ...
  // ─── /quote picker card (read another topic in this chat) ───────────────
  'card.quote.title': '💬 Pick a topic to read',
  'card.quote.empty': 'No other topics found in this chat\'s recent messages.\n(Only the recent chat tail is scanned — much older topics fall outside the window.)',
  ...
  'card.quote.btn_confirm': 'Read this topic',
  'card.quote.btn_confirm_with_task': 'Read it and run my instruction',
  ...
  'cmd.close.worktree_thread_only': '⚠️ `/close wt` is only for child topics created by `/tw`. Use `/close` for the top-level chat session.',
  'cmd.close.worktree_not_linked': '⚠️ The current session directory is not a linked worktree; refusing to delete it. Use /close for a normal session.',
  'cmd.close.worktree_removed': '🧹 Closed this session, additionally closed {count} sessions sharing the worktree, and removed worktree: `{path}`',
  'cmd.close.worktree_confirm_title': 'Confirm close topic and remove worktree',
  'cmd.close.worktree_confirm_sessions': 'Sessions to close: {count} including this one',
  'cmd.close.worktree_confirm_effect': 'After removal: uncommitted changes are lost; unpushed commits stay on the local branch, but the worktree directory is removed.',
  'cmd.close.worktree_confirm_button': 'Confirm close and remove',
  'cmd.close.worktree_confirm_no_perm': 'Only a bot operator can close sessions and remove a worktree.',
  'cmd.close.worktree_confirm_not_invoker': 'Only the user who requested this confirmation can execute it.',
  'cmd.close.worktree_confirm_received': 'Request processed. Check the topic for the result.',
  'cmd.close.worktree_state_changed': '⚠️ The worktree changed after this confirmation card was created. No session was closed and no directory was removed. Review the fresh card below and retry.',
  'cmd.close.worktree_confirm_path_label': 'Worktree to remove',
  'cmd.close.worktree_col_bot': 'Bot',
  'cmd.close.worktree_col_task': 'Current task',
  'cmd.close.worktree_current_tag': '·current',
  'cmd.close.worktree_bot_unknown': 'unknown bot',
  'cmd.close.worktree_checks_label': 'Safety checks',
  'cmd.close.worktree_check_dirty_ok': '✅ Uncommitted changes: none',
  'cmd.close.worktree_check_dirty_warn': '⚠️ Uncommitted changes: {n} file(s)',
  'cmd.close.worktree_check_ahead_ok': '✅ Unpushed commits: none',
  'cmd.close.worktree_check_ahead_warn': '⚠️ Unpushed commits: {n}',
  'cmd.close.worktree_effect_safe': 'This worktree is clean; removing it will not lose any code.',
  'cmd.close.worktree_confirm_risky': '⚠️ About to close sessions and remove this worktree:\n- Other active sessions sharing it: {count}\n- Uncommitted changes: {dirty}\n- Unpushed commits: {ahead}\nTo continue, send `/close wt --yes`.',
  'cmd.close.worktree_confirm_shared': '⚠️ {count} active session(s) are also using this worktree. To close them all and remove the worktree, send `/close wt --yes`.',
  'cmd.close.worktree_sibling_close_failed': '⚠️ The current session was closed, but {count} other session(s) using `{path}` could not be closed. The worktree was not removed; retry from a remaining session after its owner daemon recovers.',
  'cmd.close.worktree_remove_failed': '⚠️ Session closed, but failed to remove worktree `{path}`: {error}',
  ...
  'cmd.repo.here_missing': '⚠️ This chat has no current working directory to reuse. Start a normal chat session first, or bind one with /oncall bind.',
  ...
  'cmd.login.lark_step1': 'Open the link below and confirm with your own Feishu account:',
  'cmd.login.lark_step2': '2. After authorizing, send /login done here',
  'cmd.login.lark_note': 'This is your own lark-cli identity for reading Feishu (docs / drive / wiki / sheets). Authorize just once; it auto-renews for about 7 days with no re-tap, and you only tap again after it expires. It uses this service’s configured app; everyone in the group authorizes separately, isolated from each other.',
  'cmd.login.lark_pending': '⏳ Authorization not detected yet. Open the link above, then send /login done.',
  'cmd.login.lark_ok': '✅ lark-cli authorized. From now on your lark-cli Feishu reads run with your own identity.',
  'cmd.login.lark_failed': '❌ lark-cli authorization failed: {detail}. Send /login lark to try again.',
  'cmd.login.lark_no_challenge': '❌ No lark-cli authorization in progress. Send /login lark to get a link.',
  'cmd.login.lark_begin_failed': '❌ Could not start lark-cli authorization: {detail}',
  'cmd.login.lark_status_yes': 'lark-cli (your Feishu identity): authorized',
  'cmd.login.lark_status_no': 'lark-cli (your Feishu identity): not authorized — send /login lark and tap the link',
  ...
  'trigger_user_auth.denied_auto_login': 'botmux: this step needs YOUR OWN {tool} ({provider}) permission. Authorize just once (it then renews automatically — no repeated taps) by opening:',
  'trigger_user_auth.denied_auto_retry': 'Then just ask me to retry — no command to type. Not tapping means not authorizing, and this step will not run (nor as the bot or anyone else).',
  ...
  'cmd.project.no_bot': '⚠️ Cannot identify the responding bot — /project aborted.',
  'cmd.project.no_chat': '⚠️ Cannot identify the current chat — /project aborted.',
  'cmd.project.no_owner': '⚠️ This bot has no allowedUsers (no owner), so project-group configuration cannot be changed from chat. Configure an admin in Dashboard or on the host first.',
  'cmd.project.not_admin': '⛔ Only owner / allowedUsers may change project-group configuration.',
  'cmd.project.help': '📌 /project — manage project collaboration mode for this ordinary group (owner/allowedUsers only)\n\n@bot /project enable   — use this bot as coordinator and other local bots as workers; future bots auto-enroll\n@bot /project status   — show configuration and project status\n@bot /project roles    — configure the coordinator and worker roles for this group\n@bot /project disable  — leave project-group mode\n\nA settled goal is not required when enabling. Start with a top-level discussion with the coordinator, then ask it to initialize the project when the goal is ready. The worker list and auto-enrollment policy can be adjusted in Dashboard.',
  'cmd.project.chat_lookup_failed': '⚠️ Could not confirm the chat type, so no configuration was changed. Try again shortly.',
  'cmd.project.ordinary_group_required': '⚠️ /project works only in ordinary group chats; DMs, topic-mode groups, and session groups are unsupported.',
  'cmd.project.bot_roster_unavailable': '⚠️ Could not read the group bot roster, so no configuration was changed. Make sure the bots are in the group and retry.',
  'cmd.project.coordinator_not_in_chat': '⚠️ The current coordinator bot was not found in the live group roster. Make sure it has joined this group and retry.',
  'cmd.project.coordinator_conflict': '⛔ Another coordinator already owns this group ({value}). Have it run /project disable first, or change the setting in Dashboard.',
  'cmd.project.project_mode_required': '⚠️ Project-group mode is not enabled here. Send: @bot /project enable',
  'cmd.project.unknown_subcommand': 'Unknown subcommand: {value}\nSupported: /project enable | status | roles | disable | help',
  'cmd.project.unexpected_arguments': 'This /project subcommand takes no extra arguments: {value}\nSupported: /project enable | status | roles | disable | help',
  'cmd.project.failed': '⚠️ Project-group configuration failed: {reason}',
  'cmd.project.none': 'none',
  'cmd.project.waiting': 'waiting to start (discussion is fine)',
  'cmd.project.worker_policy_auto': 'automatically enroll newly joined local bots (can be disabled in Dashboard)',
  'cmd.project.worker_policy_manual': 'use only the explicit worker list',
  'cmd.project.enabled': '✅ Project-group mode enabled\nCoordinator: {coordinator}\nWorkers: {workers}\nWorker policy: {workerPolicy}\n\nThe collaboration policy is active immediately, and the getting-started card has been sent and pinned. Continue in the top-level chat by mentioning the coordinator with a discussion or goal.',
  'cmd.project.reenabled': '✅ Project-group mode re-enabled\nCoordinator: {coordinator}\nWorkers: {workers}\nWorker policy: {workerPolicy}\n\nThe existing project state and progress card were retained, so work can continue.',
  'cmd.project.already_enabled': '✅ This group is already in project mode\nCoordinator: {coordinator}\nWorkers: {workers}\nWorker policy: {workerPolicy}\n\nThe existing worker selection and auto-enrollment policy were preserved by this repeated command.',
  'cmd.project.card_deferred': '⚠️ The configuration is active, but the getting-started card was not refreshed. Retry /project enable later.',
  'cmd.project.status_standard': 'This group is in standard collaboration mode.\n\nSend in the top-level chat: @bot /project enable',
  'cmd.project.status_project': '📌 Project-group mode is active\nCoordinator: {coordinator}\nWorkers: {workers}\nWorker policy: {workerPolicy}\nProject: {runtime}',
  'cmd.project.disabled': '✅ Project-group mode disabled; any unused pinned getting-started card was removed.',
  'cmd.project.already_disabled': 'This group is not in project mode; nothing to disable.',
  'cmd.project.project_retained': 'Existing project state and progress card were retained; re-enable the same coordinator to continue.',
  ...
  'help.quote': '/quote [instruction] - Pick another topic in this chat and read its transcript into the current session (Feishu can only quote a single message; this quotes a whole topic). Without an instruction it reads, acknowledges and waits; with one it acts immediately',
  ...
  'help.sessions': '/sessions   - List this bot\'s topic sessions in the current group and open or resume a topic',
  ...
  'help.project': '/project enable|status|disable - Enable, inspect, or leave project-group mode in this ordinary group (owner only)',
  ...
  'ai.credentials.on_auth_failure': 'On an auth failure, report it as-is. A missing ByteCloud authorization includes an automatically generated login link on stderr: give that link to the person and retry the original action after they authorize; mention /login bytedcli only when no link was returned. Feishu authorization still uses /login. Do not look for, assemble, or reuse other credentials to work around it.',
  ...
  'ai.credentials.on_auth_link': 'If a command is refused and stderr contains an authorization link (accounts.feishu.cn/device/verify or a ByteCloud/cloud.bytedance link) plus instructions to authorize and retry: immediately relay that text and the link verbatim to the user with botmux send (forward the link byte-for-byte; do not rewrite, encode, or add punctuation; keep the "authorize once, auto-renews" note so they know it is a one-time tap), then stop and wait. Do not retry under any other identity and do not pretend the operation succeeded. The user will call you again after authorizing; retry then.',
  ...
  'daemon.force_topic_current_dir_missing': '⚠️ This chat has no current working directory to reuse. Start a normal chat session first, or bind one with /oncall bind.',
  ...
  'daemon.topic_header_ready_effort': 'reasoning effort {effort}',
  'daemon.topic_header_rejected': '⚠️ Something is wrong with the topic header, so no topic was opened:\n{reasons}\n\n{usage}',
  'daemon.topic_header_usage': 'Usage: `[title] /t [/repo <repo>] [/model <model>] [/effort <level>] <first task>` (newlines are the same as spaces; quote paths containing spaces).',
  'daemon.topic_header_missing_arg': '`{directive}` is missing its argument',
  'daemon.topic_header_duplicate': '`{directive}` appears twice; each directive may be used only once',
  ...
  'daemon.topic_header_effort_unsupported_cli': 'This CLI ({cli}) has no reasoning-effort control',
  'daemon.topic_header_effort_unsupported_model': 'Model {model} does not support reasoning effort {effort}',
  ...
  'card.group_sessions.summary': 'Active {active} · Closed {closed} · Page {page}/{totalPages}',
  'card.group_sessions.empty': 'There are no topic sessions for this bot in the current group.',
  'card.group_sessions.untitled': 'Untitled topic',
  'card.group_sessions.status_unknown': 'Unknown status',
  'card.group_sessions.status_closed': 'Closed',
  'card.group_sessions.time.just_now': 'just now',
  'card.group_sessions.time.seconds': '{n}s ago',
  'card.group_sessions.time.minutes': '{n}m ago',
  'card.group_sessions.time.hours': '{n}h ago',
  'card.group_sessions.time.days': '{n}d ago',
  'card.group_sessions.open_topic': '↗ Open topic',
  'card.group_sessions.locate_topic': '📍 Locate topic',
  'card.group_sessions.prev': '← Prev',
  'card.group_sessions.next': 'Next →',
  'card.group_sessions.refresh': '🔄 Refresh',
  'card.group_sessions.resume': '▶ Resume',
  'card.group_sessions.resume_confirm_title': 'Resume this session?',
  'card.group_sessions.resume_confirm_text': 'Resume it to continue in the original topic. Session: {title}',
  'card.group_sessions.footer': 'Data as of {time}; refresh for the latest status. Working directories, full session IDs, and terminal links are hidden.',
  'card.group_sessions.unavailable': '⚠️ Session list is unavailable in this conversation.',
  'card.group_sessions.group_only': '`/sessions` is available in group chats only.',
  'card.group_sessions.list_failed': '⚠️ Failed to fetch topic sessions: {reason}',
  'card.group_sessions.not_invoker': 'Only the person who opened this card can operate it.',
  'card.group_sessions.invalid_action': '⚠️ Invalid card action.',
  'card.group_sessions.binding_invalid': '⚠️ This card is no longer bound to the current group.',
  'card.group_sessions.session_not_found': '⚠️ This topic session moved or is no longer available.',
  'card.group_sessions.locate_success': '📬 A locate ping was sent to the original topic.',
  'card.group_sessions.locate_failed': '⚠️ Failed to locate the topic: {reason}',
  'card.group_sessions.resume_owner_only': 'Only bot admins can resume sessions.',
  'card.group_sessions.resume_unavailable': '⚠️ This session was resumed, moved, or is no longer resumable.',
  'card.group_sessions.resume_failed': '⚠️ Failed to resume the session: {reason}',
  'card.group_sessions.resume_success': '✅ Session resumed.',
  ...
  'card.dashboard.groups.project_roles.title': '🎭 Project group roles',
  'card.dashboard.groups.project_roles.help': 'Configure each agent’s responsibility in this group. Goals, progress, and tasks do not belong here.',
  'card.dashboard.groups.project_roles.coordinator': 'Coordinator',
  'card.dashboard.groups.project_roles.worker': 'Worker',
  'card.dashboard.groups.project_roles.inherited': 'No group role; this bot inherits its team default role when available',
  'card.dashboard.groups.project_roles.edit': 'Edit group role',
  'card.dashboard.groups.project_roles.empty': 'No editable agents are configured for this project.',
  'card.dashboard.groups.project_roles.footer': 'Uses the same group-role source as /role and Dashboard. Every-turn mode applies on the next message; once mode applies after a new or rebuilt session.',

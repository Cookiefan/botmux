...
describe('DAEMON_COMMANDS set', () => {
  it('should contain all expected commands', () => {
    const expected = ['/close', '/restart', '/status', '/retry', '/help', '/cd', '/repo', '/rename', '/schedule', '/role', '/botconfig', '/skills', '/pair', '/login', '/adopt', '/detach', '/disconnect', '/oncall', '/group', '/g', '/relay', '/quote', '/fork', '/forklist', '/card', '/cot', '/term', '/list-slash-command', '/slash', '/subscribe-lark-doc', '/watch-comment', '/vc', '/insight', '/dashboard', '/sessions', '/vc-auth', '/issue', '/cli'];
  ...
  it('should have the correct size', () => {
    // 38 = master 的 36 条 + /quote + /sessions。
    // /fork 与 /issue 仍是一等 daemon 命令；/subscribe-lark-doc 保持原本的
    // 按文件 API 订阅命令语义，不做别名。
    expect(DAEMON_COMMANDS.size).toBe(38);
  });

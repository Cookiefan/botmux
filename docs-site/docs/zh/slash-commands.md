...
- `/repo wt <路径|项目名> [分支]` —— 先在该仓库上新建一个 worktree（基于远端默认分支），再在里面开会话。分支可省略（按标题/首轮任务自动命名）；写了分支时**只认与仓库同一行、长得像分支名的下一个词**（`ci/temp_split` 这类），中文正文不会被误吃，但英文开头的正文请换行再写。分支名不合法、目标目录已存在都会在开话题前拒绝；git 侧创建失败则话题已建、会话停在待选仓状态，在话题内重发 `/repo wt …` 即可。
